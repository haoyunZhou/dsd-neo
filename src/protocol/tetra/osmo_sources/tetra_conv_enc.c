...
/* Minimal compatibility shim for osmo-tetra source when built in-tree.
 * Provide small stubs for DEBUGP, ARRAY_SIZE and osmo_hexdump used by the
 * copied file so we don't need the full osmocom tree headers here.
 */
#ifndef DEBUGP
...
static inline const char* osmo_hexdump(const uint8_t *buf, int len) { (void)buf; (void)len; return ""; }
